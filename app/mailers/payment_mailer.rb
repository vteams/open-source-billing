...
class PaymentMailer < ActionMailer::Base
  ...
  def get_email_template(user = nil, invoice, template_type)
    ...
    template  = user.accounts.first.email_templates.where(:template_type => template_type).first if template.blank? && user.present?
    template
  end
  ...
  def replace_template_body(user = nil, payment, template_type)
    ...
    param_values = {
        ...
        'invoice_url' => "#{Osb::Application.config.address}/invoices/preview?inv_id=#{invoice.encrypted_id}"
    }
