...
window.applyChosen = (single_row) =>
  ...
  $(".chzn-select:not('.invoice_company, .company_filter, .frequency_select, .per_page, .discount_select, .invoice_currency, .recurring_profile_currency, .report_billing_method, .expense_category')",'.estimates-main, .projects-main, .invoices-main,.recurring_profiles-main, .expenses-main').on "liszt:ready", ->
    chzn_drop = $(this).next().find(".chzn-drop")
  ...
  identical_line_items = $('.invoice_grid_fields tr.fields:last .chzn-select:first option, .task_grid_fields tr.fields:last .chzn-select:first option, .estimate_grid_fields tr.fields:last .chzn-select:first option')
  ...
  dropdowns = unless single_row? then $(".chzn-select") else $(single_row)
  ...
  $(".add-new").on "click",null,(event) ->
    console.log $(this).attr("data-dropdown-id")
    new InlineForms($(this).attr("data-dropdown-id")).showForm()
...
#    $("#invoice_discount_amount_lbl").text(discount_amount.toFixed(2))
  ...
  $(".invoice_grid_fields").on "change",'select.items_list', ->
  ...
  $(".invoice_grid_fields").on "click",'.add_nested_fields', ->
    setTimeout "window.applyChosen($('.invoice_grid_fields tr.fields:last .chzn-select'))", 0
  ...
  $("#invoice_invoice_date, #invoice_due_date, .date_picker_class").datepicker
  ...
  $('.use_as_template ').on "click",'.close_qtip', ->
    hidePopover($("#invoice_client_id_chzn"))

  $("#invoice_client_id_chzn,.chzn-container").on "click",null, ->
    $(this).qtip("hide")
  ...
  $("#invoice_client_id").unbind 'change'
  $("#invoice_client_id").change ->
    client_id = $(this).val()
    hidePopover($("#invoice_client_id_chzn")) if client_id is ""
    $("#last_invoice").hide()
      ...
      jQuery.ajax '/clients/get_last_invoice',
        ...
        success: (data, textStatus, jqXHR) ->
          ...
          unless id is "no invoice"
            useAsTemplatePopover($("#invoice_client_id_chzn"),id,client_name)
          else
  ...
  $("input[type=text],input[type=number],input[type=checkbox]",".quick_create_wrapper").on("focus",null,->
  ...
  $(".single-recover-link").on "click",null, ->
