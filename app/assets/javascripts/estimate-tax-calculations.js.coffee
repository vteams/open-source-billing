...
window.taxByCategory = ->
  taxes = []
  jQuery("table.estimate_grid_fields tr:visible").each ->
    # TODO: apply discount on lineTotal
    discountPct = parseFloat($("#estimate_discount_percentage, #recurring_profile_discount_percentage").val())
  ...
  jQuery(".grid_summary_row.taxes_total").html("<ul>#{lis}</ul>").parents(".grid_summary").find("#estimate_sub_total_lbl, #estimate_discount_amount_lbl, .tax_amount").formatCurrency({symbol: window.currency_symbol});