...
node(:max_quantity) {|m| Company.find(@current_user.current_company).items.maximum('quantity').to_f }
node(:max_item_cost) {|m| Company.find(@current_user.current_company).items.maximum('unit_cost').to_f }
child(@items) do
