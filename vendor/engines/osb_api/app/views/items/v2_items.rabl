...
node(:max_quantity) {|m| company.items.count > 0 ? company.items.with_deleted.maximum('quantity').to_f : 0.0 }
node(:max_item_cost) {|m| company.items.count > 0 ? company.items.with_deleted.maximum('unit_cost').to_f : 0.0 }
child(@items) do
