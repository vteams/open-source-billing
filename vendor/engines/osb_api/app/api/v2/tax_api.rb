module V2
  class TaxApi < Grape::API
    version 'v2', using: :path, vendor: 'osb'
    format :json
    #prefix :api

    resource :taxes do
      before {current_user}

      desc 'Return all taxes'
      get do
        @taxes = Tax.all.order("#{params[:sort].present? ? params[:sort] : 'name'} #{params[:direction].present? ? params[:direction] : 'asc'}").page(params[:page]).per(@current_user.settings.records_per_page)
        @taxes = {total_records: @taxes.total_count, total_pages: @taxes.total_pages, current_page: @taxes.current_page, per_page: @taxes.limit_value, taxes: @taxes}
      end

    end
  end
end



